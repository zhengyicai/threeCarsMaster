...
public interface UseCommunityMapper extends BaseMapper<UseCommunityPo>{
	 ...
	 * @return
	 */
	@Select("select c.*,re.name,re.mobile, u.userName as userName,u.state as userStatus,u1.userName as userWorkName,u1.state as userWorkStatus from use_community c left join sys_user u on u.id = c.sysUserId  left join use_resident  re on c.residentId = re.id  left join   sys_user u1 on u1.id =c.sysWorkId  inner join  use_community_user  ucu  where c.id = ucu.communityId and ucu.userId=#{userId} order by c.createTime desc")
	...
	@Select("select distinct city from use_community where state = '10'")
	...
	@Select("select DISTINCT area,city from use_community where state = '10' ")
	public  List<UseCommunityPo> findAreas();

	@Select("select DISTINCT address,area from use_community where state = '10' ")
	public  List<UseCommunityPo> findAddresss();

	@Select("SELECT * from use_community where area = #{area} and address=#{address} and state = '10' limit 1")
